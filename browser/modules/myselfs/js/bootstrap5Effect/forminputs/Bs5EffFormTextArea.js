 ...
 * 
 * @file This file "Bs5EffFormTextArea.js" is part of project "js-learning" , which is belong to Michael Pang (It's Me).
 * @author  Micheal Pang (Dongcan Pang)
 * @createDate  2025-08-28
 * @version 1.0.0 
 * @description  这是关于 表单 多行文字 输入框的一个组件。它由 FormInput 派生出来的表单输入组件。
 */
"use strict"; // 这是严格模式下的 Javascript 代码

import { Bs5EffFormInput, protected_set_MyInputComponent } from "./Bs5EffFormInput.js";
import { Bs5EffTextArea } from "../inputs/Bs5EffTextArea.js";
import { ValidUtil as vu } from "../../../../../utils/valid.js";
import { myRandomNumStr } from "../../bootstrap5UI.js";

/**
 * 表单 多行文字 输入组件。继承自 Bs5EffFormInput
 */
class Bs5EffFormTextArea extends Bs5EffFormInput {
     ...
     * @param {number} [comptconfig.rows] 多行文本的行数，一般用于显示的高度处理。默认是 3 。
     ...
     */
    constructor(id='fTxArea'+myRandomNumStr(), 
        { formCssClass='', labelInfo='', helperInfo='', invalidInfo='' }={},
        { rows=3, cssClass='', defaultValue='', placeholder='', validRule=undefined, customEvent=undefined }={}
    ){
        // 参数验证（优化）这里校验可配置参数，是否有传递，或者类型是否错误（当 options 为 undefined 则会自动以默认值填充）
        vu.autoVnAofObjectLiteral(arguments[1], 'Bs5EffFormTextArea', 'constructor', 'formconfig', true);
        vu.autoVnAofObjectLiteral(arguments[2], 'Bs5EffFormTextArea', 'constructor', 'comptconfig', true);
        let idVal = vu.autoVnAofString(id, 'Bs5EffFormTextArea', 'constructor', 'id', false);
        
        // 父类初始化
        super(idVal, arguments[1], arguments[2]);
        
        // 将内部组件置换 为 Bs5EffTextArea
        this[protected_set_MyInputComponent](new Bs5EffTextArea(idVal, arguments[2]));
    }
